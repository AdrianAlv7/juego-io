...
import {
  GARAGE_MOTOS,
  DEFAULT_GARAGE_MOTO_ID,
  getGarageMotoById,
} from "../src/garage/catalog.js";
...
function sanitizeMotoId(value) {
  return getGarageMotoById(
    typeof value === "string" ? value.trim().toLowerCase() : DEFAULT_GARAGE_MOTO_ID
  ).id;
}

function sanitizeOptionalMotoId(value) {
  if (typeof value !== "string" || !value.trim()) return "";
  return sanitizeMotoId(value);
}

function pickDefaultMotoIdForRoom(room) {
  const usedMotoIds = new Set(
    Array.from(room.players.values(), (player) => sanitizeMotoId(player.motoId))
  );
  const availableMotoIds = GARAGE_MOTOS.map((moto) => moto.id).filter(
    (motoId) => !usedMotoIds.has(motoId)
  );
  const motoPool = availableMotoIds.length
    ? availableMotoIds
    : GARAGE_MOTOS.map((moto) => moto.id);
  return motoPool[randomInt(0, motoPool.length - 1)] || DEFAULT_GARAGE_MOTO_ID;
}

function resolveInitialMotoId(room, requestedMotoId = "") {
  return requestedMotoId ? sanitizeMotoId(requestedMotoId) : pickDefaultMotoIdForRoom(room);
}

function ensureLobbyMotoAssignments(room) {
  const usedMotoIds = new Set();
  const deferredPlayers = [];

  for (const player of room.players.values()) {
    const normalizedMotoId = sanitizeMotoId(player.motoId);
    if (player.manualMoto) {
      player.motoId = normalizedMotoId;
      usedMotoIds.add(normalizedMotoId);
      continue;
    }
    if (!usedMotoIds.has(normalizedMotoId)) {
      player.motoId = normalizedMotoId;
      usedMotoIds.add(normalizedMotoId);
      continue;
    }
    deferredPlayers.push(player);
  }
  ...
  for (const player of deferredPlayers) {
    ...
    const nextMotoId = availableMotoIds.length
      ? availableMotoIds[randomInt(0, availableMotoIds.length - 1)]
      : sanitizeMotoId(player.motoId);
    player.motoId = nextMotoId;
    usedMotoIds.add(nextMotoId);
  }
}
...
function getLobbyPlayers(room) {
  ensureLobbyMotoAssignments(room);
...
function sanitizeRoomJoinPayload(payload = {}) {
  ...
  const motoId = sanitizeOptionalMotoId(payload.motoId);
  return {
    roomMode,
    name,
    roomCode,
...
function buildPlayerRecord(
  name,
  motoId = DEFAULT_GARAGE_MOTO_ID,
  manualMoto = false
) {
  return {
    name,
    motoId: sanitizeMotoId(motoId),
    manualMoto: Boolean(manualMoto),
    ready: false,
...
function assignPlayerToRoom(room, socket, payload = {}) {
  ...
  const safeName =
    typeof payload.name === "string" && payload.name.trim()
      ...
      : `Jugador ${number}`;
  const assignedMotoId = resolveInitialMotoId(room, payload.motoId);

  socket.join(getRoomChannel(room.id));
  room.players.set(
    socket.id,
    buildPlayerRecord(safeName, assignedMotoId, Boolean(payload.motoId))
  );
...
io.on("connection", (socket) => {
  ...
  socket.on("setGarageMoto", (payload = {}) => {
    const room = getRoomBySocketId(socket.id);
    if (!room || room.started || room.finished) return;
    ...
    const nextMotoId = sanitizeMotoId(payload.motoId);
    if (player.motoId === nextMotoId) return;

    player.motoId = nextMotoId;
    player.manualMoto = true;
    emitLobbyState(room);
  });
